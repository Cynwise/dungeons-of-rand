...
void from_json(const Json& json, Actor_Module& mod)
{
    ...
    it = json.find("agility");
    if (it != json.end())
    {
        json.at("agility").get_to(mod.agility);
    }

    it = json.find("dexterity");
    if (it != json.end())
    {
        json.at("dexterity").get_to(mod.dexterity);
    }
    ...
    it = json.find("weapon_list");
    if (it != json.end())
    {
        json.at("weapon_list").get_to(mod.weapon_list);
    }
